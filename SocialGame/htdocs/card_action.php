    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        ...
        switch ($action) {
            ...
            case 'strengthen':
                ...
                // レベルアップ判定
                $stmt2 = $pdo->prepare("SELECT required_exp FROM exp_table WHERE level=?");
                ...
                while (true) {
                    ...
                    if (!$row) break; // レベル上限
                    $req = $row['required_exp'];
                    if ($remaining_exp >= $req) {
                        $level++;
                        $remaining_exp -= $req;
                    } else {
                        break;
                ...
                $stmt->execute([$remaining_exp, $level, $target_id]);
                ...
                header("Location: user_cards_list.php?strengthen_result=success"
                    ...
                    . "&old_hp={$old_hp}&new_hp={$old_hp}" // 現状base値のみ
